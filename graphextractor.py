...
def run_joern(filepath: str, verbose: int):
    ...
    script_file = external_dir() / "get_func_graph.scala"
    filename = external_dir() / filepath
    params = f"filename={filename}"
    command = f"joern --script {script_file} --param '{params}'"#f"joern --script {script_file} --params='{params}'" 
    command = str(external_dir() / "joern-cli" / command)
    if verbose > 2:
        debug(command)
    subprocess_cmd(command, verbose=verbose)
    try:
        shutil.rmtree(external_dir() / "joern-cli" / "workspace" / filename.name)
...
def full_run_joern_from_string(code: str, dataset: str, iid: str, verbose=0):
    ...
    savedir = get_dir(interim_dir() / dataset)

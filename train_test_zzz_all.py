...
def read_CWEvuldescription(_idd):
    """This function read domain information from the description folders,
    collected from the Mitre 1000"""
    pathd = f"{processed_dir()}/bigvul/CWEdescription"
...
# from the samples functions
def read_CWESample(_idd):
    """This function read domain information from the description folders,
    sample functions from the Mitre 1000"""
    pathd = f"{processed_dir()}/bigvul/CWE_Samples"
...
class BigVulDatasetLineVD(BigVulDataset):
    ...
    def item(self, _id, codebert=None):
        ...
        desccv = read_CWEvuldescription(_idd = _id)
        textcw = [desccv]
        embedd_textcwe = cb.encode(sents = textcw).detach().cpu()
        g.ndata['_CWEVuldesc'] = th.Tensor(embedd_textcwe).repeat((g.number_of_dst_nodes(), 1))
        ...
        cos_simalarity = cosine_similarity(g.ndata["_FUNC_EMB"], g.ndata['_CWESample'])
        max_cos = np.max(cos_simalarity)
        # 
